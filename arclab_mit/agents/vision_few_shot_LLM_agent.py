...
from arclab_mit.agents.other.update_preamble_examples import update_preamble_with_examples
...
class VisionLLMAgent(SimpleLLMAgent):
    ...
    def __init__(self, prompts=None, log_file_prefix="vision_LLM_agent_log", is_few_shot=True, **kwargs):
        ...
        # Load preamble with examples during initialization
        try:
            self.preamble = update_preamble_with_examples()
            if self.preamble is not None:
                print(colored("✓ Loaded preamble with examples", "green"))
            else:
                print(colored("⚠ No examples found in preamble", "yellow"))
        except Exception as e:
            print(colored(f"❌ Failed to load preamble with examples: {e}", "red"))
            self.preamble = None
    ...
    def get_completion(self, prompt, image=None, model=None):
        ...
        # Initialize messages with default system prompt
        messages = [
            {"role": "system", "content": self.system_prompt},
        ]

        # Try to use preamble if available
        if self.preamble is not None:
            try:
                # Verify the preamble has valid example images
                has_valid_example = False
                for message in self.preamble["messages"]:
                    if (message["role"] == "user" and 
                        len(message["content"]) > 1 and 
                        "image_url" in message["content"][1] and 
                        message["content"][1]["image_url"]["url"].startswith("data:image/jpeg;base64,")):
                        has_valid_example = True
                        break
                
                if has_valid_example:
                    messages = self.preamble["messages"]
                    print(colored("✓ Using preamble with examples", "green"))
                else:
                    print(colored("⚠ Preamble missing valid example images", "yellow"))
            except Exception as e:
                print(colored(f"⚠ Error processing preamble: {str(e)}", "yellow"))
                
        # Add the current screenshot with standard question
        if image is not None:
            image_b64 = self.prepare_image(image)
            print(colored("\n=== IMAGE INFO ===", "cyan"))
            print(colored("Base64 string length: " + str(len(image_b64)), "yellow"))
            
            # Add the current screenshot message
            messages.append({
                "role": "user",
                "content": [
                    {"type": "text", "text": "What is the best throttle to approach the lady?"},
                    {
                        "type": "image_url",
                        "image_url": {
                            "url": image_b64,
        ...
        # Add the current prompt as a separate message
