...
from kspdg.pe1.e3_envs import PE1_E3_I1_Env, PE1_E3_I2_Env, PE1_E3_I3_Env, PE1_E3_I4_Env
from kspdg.sb1.e1_envs import SB1_E1_I1_Env, SB1_E1_I2_Env, SB1_E1_I3_Env, SB1_E1_I4_Env, SB1_E1_I5_Env
from kspdg.lbg1.lg1_envs import LBG1_LG1_I1_Env, LBG1_LG1_I2_Env
from kspdg.lbg1.lg2_envs import LBG1_LG2_I1_Env, LBG1_LG2_I2_Env
...
class State:
    ...
    def to_json(self, scenario, use_relative_coordinates=False, use_short_names=False, use_prograde=False,
        ...
        state_json = {
            "vehicle_mass": float(self.vehicle_mass),
            "vehicle_propellant": float(self.vehicle_propellant),

            "pursuer_pos_x": float(self.pursuer_position[0]),
            "pursuer_pos_y": float(self.pursuer_position[1]),
            "pursuer_pos_z": float(self.pursuer_position[2]),

            "pursuer_vel_x": float(self.pursuer_velocity[0]),
            "pursuer_vel_y": float(self.pursuer_velocity[1]),
            "pursuer_vel_z": float(self.pursuer_velocity[2]),

            "evader_pos_x": float(self.evader_position[0]),
            "evader_pos_y": float(self.evader_position[1]),
            "evader_pos_z": float(self.evader_position[2]),

            "evader_vel_x": float(self.evader_velocity[0]),
            "evader_vel_y": float(self.evader_velocity[1]),
            "evader_vel_z": float(self.evader_velocity[2]),
        }
        if scenario.startswith('sb'):
            ...
            # Replace names
            state_json["bandit_pos"] = [float(x) for x in [state_json.pop('pursuer_pos_x'),
                                        state_json.pop('pursuer_pos_y'),
                                        state_json.pop('pursuer_pos_z')]]

            state_json["bandit_vel"] = [float(x) for x in [state_json.pop('pursuer_vel_x'),
                                        state_json.pop('pursuer_vel_y'),
                                        state_json.pop('pursuer_vel_z')]]

            state_json["lady_pos"] = [float(x) for x in [state_json.pop('evader_pos_x'),
                                      state_json.pop('evader_pos_y'),
                                      state_json.pop('evader_pos_z')]]

            state_json["lady_vel"] = [float(x) for x in [state_json.pop('evader_vel_x'),
                                      state_json.pop('evader_vel_y'),
                                      state_json.pop('evader_vel_z')]]

            state_json["guard_pos"] = [float(x) for x in self.guard_position]
            state_json["guard_vel"] = [float(x) for x in self.guard_velocity]
    ...
    #
    # Returns an action which aligns prograde while limiting the approach speed
    #
    def get_recommendation(self):
        ...
        """
        retrograde = self.get_retrograde(ref_frame="vessel")
        ...
        if (abs(v[0]) > ROTATION_THRESHOLD) or (abs(v[2]) > ROTATION_THRESHOLD):
            ...
            rt = 0
            dt = 0
        if (np.dot(self.rel_position, self.rel_velocity) < 0) and (
            self.time_to_intercept * VESSEL_ACCELERATION < self.velocity) and (
            self.distance > EVASION_DISTANCE):
            # Target is approaching and intercept time is insufficient to stop
            if self.velocity > APPROACH_SPEED:
                # Reduce speed
                ft = -1
            else:
                ft = 0

        action = [ft, rt, dt, DEFAULT_DURATION]
        return action
...
def orbital_from_vectors(mu, r, v):
    ...
    return np.array([sma, ecc, inc, lpe, lan, mna])
